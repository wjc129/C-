...
# 定义用于 curve_fit 的函数
def steinmetz_combined(xdata, k1, alpha1, beta1, c, d, e,x):
    f, Bm, T = xdata
    return steinmetznew(f, Bm, k1, alpha1, beta1, c, d, e, x, T)
...
# 打印拟合结果
print(f"Fitted parameters: k1={popt[0]}, alpha1={popt[1]}, beta1={popt[2]}, c={popt[3]}, d={popt[4]}, e={popt[5]}, x={popt[6]}")

# 使用拟合的参数生成预测值
fitted_values = steinmetznew(f_data, Bm_data, popt[0], popt[1], popt[2], popt[3], popt[4], popt[5], popt[6],T_data)
...
# 绘制拟合数据的竖线，使用 vlines
plt.vlines(sample_indices, 0, fitted_values[sample_indices], color='purple', linestyle='solid', label='混合修正拟合磁芯损耗')
