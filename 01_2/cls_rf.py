...
rf = RandomForestClassifier(n_estimators=100, random_state=42)  # 设置树的数量
...
y_pred_ts = rf.predict(scaled_features_ts)
...
# 保存结果到 Excel 文件
df_4.to_excel('./01_2/附件四（Excel表）_rf.xlsx', index=False)

# 总耗时
end_time = time.time()
print("预测结果已保存至附件四（Excel表）_rf.xlsx")
