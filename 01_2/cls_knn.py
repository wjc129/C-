...
from sklearn.neighbors import KNeighborsClassifier  # 修改为KNN
...
knn = KNeighborsClassifier(n_neighbors=5)  # 设置邻居数，可以根据需要调整
...
y_pred_ts = knn.predict(scaled_features_ts)
...
# 保存结果到 Excel 文件
df_4.to_excel('./01_2/附件四（Excel表）_knn.xlsx', index=False)

# 总耗时
end_time = time.time()
print("预测结果已保存至附件四（Excel表）_knn.xlsx")
